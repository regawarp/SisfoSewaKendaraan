...
$query = "SELECT merk_type,COUNT(merk_type) as countMerek FROM transaksi,kendaraan WHERE transaksi.nomor_polisi=kendaraan.nomor_polisi AND ((tgl_dibuat_surat_jln  BETWEEN '$tanggal_awal' AND '$tanggal_akhir') OR tgl_dibuat_surat_jln='$tanggal_awal' OR tgl_dibuat_surat_jln='$tanggal_akhir') GROUP BY merk_type ASC";
...
while ($row = mysqli_fetch_assoc($result)) {
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('B' . $countRow, $row['countMerek']);
    $total += $row['countMerek'];
    $countRow++;
}
$objPHPExcel->setActiveSheetIndex(0)
...
// HEADER
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('A1', 'LAPORAN DATA KENDARAAN KATEGORI MEREK/TYPE')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('KATEGORI MEREK');
...
header('Content-Disposition: attachment;filename="LaporanKlienKATEGORI MEREK' . $tanggal_awal . '-' . $tanggal_akhir . '.xlsx"');
