...
$query = "SELECT penjemputan,COUNT(penjemputan) as countJemput FROM transaksi WHERE ((tgl_dibuat_surat_jln  BETWEEN '$tanggal_awal' AND '$tanggal_akhir') OR tgl_dibuat_surat_jln='$tanggal_awal' OR tgl_dibuat_surat_jln='$tanggal_akhir') GROUP BY penjemputan ASC";
$result = mysqli_query($conn, $query);
$total = 0;
$countRow = 6;
while ($row = mysqli_fetch_assoc($result)) {
    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('A' . $countRow, $row['penjemputan'])
        ->setCellValue('B' . $countRow, $row['countJemput']);
    $total += $row['countJemput'];
    $countRow++;
}
$objPHPExcel->setActiveSheetIndex(0)
...
// HEADER
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('A1', 'LAPORAN DATA KLIEN KATEGORI PENJEMPUTAN')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('KATEGORI PENJEMPUTAN');
...
header('Content-Disposition: attachment;filename="LaporanKlienKATEGORI PENJEMPUTAN' . $tanggal_awal . '-' . $tanggal_akhir . '.xlsx"');
